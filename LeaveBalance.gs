function updateLeaveBalances() {
  ...
  const otherLeaveBalancesSheet = ss.getSheetByName(OTHER_LEAVE_BALANCES); // Bereavement, Parental, Maternal
  ...
  leaveData.slice(1).forEach((row) => {
    ...
    if (
      ...
      } else if (leaveType === "Other" && subLeaveType === "Parental Leave") {
        otherLeaveBalances[email]["Parental Leave"] += days;
      } else if (leaveType === "Other" && subLeaveType === "Maternal Leave") {
        otherLeaveBalances[email]["Maternal Leave"] += days;
      }
    }
  });
  ...
  if (otherLeaveBalancesSheet) {
    ...
    otherBalancesData.slice(1).forEach((row, index) => {
      const employeeEmail = row[A_BALANCE_EMAIL];
      ...
      if (otherLeaveBalances[employeeEmail]) {
        ...
        const usedParentalLeave =
          otherLeaveBalances[employeeEmail]["Parental Leave"] || 0;
        const usedMaternalLeave =
          otherLeaveBalances[employeeEmail]["Maternal Leave"] || 0;

        // Check if employee is from @exemptedextension.com (no bereavement leave)
        const isRBAEmployee = employeeEmail.endsWith("@exemptedextension.com");
        ...
        // Only update maternal leave if employee has a valid maternal leave balance
