function onEdit(e) { 
  ...
  const updateNeeded = (
    (sheetName === ALL_RECORDS && [COL_N_EMAIL_ADDRESS, COL_N_MAIN_LEAVE, COL_N_SUB_LEAVE, COL_N_START_DATE, COL_N_END_DATE, COL_N_LEAVE_DURATION, COL_N_MAIN_STATUS].includes(editedColumn))
  );
...
function onFormSubmit(e) {
  ...
  const existingRequestId = sheet.getRange(lastRow, COL_N_REQUEST_ID).getValue();
  let requestId;
  
  if (!existingRequestId) {
    requestId = generateRequestIdEnhanced(sheet);
    sheet.getRange(lastRow, COL_N_REQUEST_ID).setValue(requestId);
    debugLog(`Generated new Request ID ${requestId} in row ${lastRow}, column Q`);
  } else {
    requestId = existingRequestId;
    debugLog(`Using existing Request ID ${requestId} in row ${lastRow}, column Q`);
  }
  ...
  const emailExists = balancesData.some(row => row[A_BALANCE_EMAIL] === employeeEmail || ALLOWED_EMAILS.includes(employeeEmail)); // Column B - Employee Email

  if (!emailExists) {
    debugLog(`Leave request automatically rejected for ${employeeName} (${employeeEmail}) due to email mismatch.`);
  ...
  if (
      !isExemptedEmail(employeeEmail) && !isParaplannerEmail(employeeEmail) && !isOtherExemptedEmail(employeeEmail) && // Apply rejection to non-exempted employees
    ...
    debugLog(`Leave request automatically rejected for ${employeeFormatName} (Start: ${leaveStartDateObj.toDateString()}, End: ${leaveEndDateObj.toDateString()})`);
          ...
          <div class="info-card-body">
              <table class="info-table" cellpadding="0" cellspacing="0">
                  <tr>
                      ...
                      <td class="info-value">${formatDate(allowedStart)} to ${formatDate(allowedEnd)}</td>
                  </tr>
                  <tr class="last-row">
                      <td class="info-label">Requested Leave:</td>
                      <td class="info-value">${formatDate(leaveStartDateObj)} to ${formatDate(leaveEndDateObj)}</td>
                  </tr>
              </table>
          </div>
      </div>
  ...
  if (balanceCheckedLeaves.includes(leaveType)) {
    ...
    const isJanToJune = leaveStartDate >= janToJuneStart && leaveEndDate <= janToJuneEnd;
    const isJulyToDec = leaveStartDate >= julyToDecStart && leaveStartDate <= julyToDecEnd;

    if (isExemptedEmail(employeeEmail)) {
      // Use "RBA Leave Balances" sheet for exempted emails
      const rbaBalancesSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(RBA_LEAVE_BALANCES);
      const rbaBalancesData = rbaBalancesSheet.getDataRange().getValues();
      
      // Find the employee in the RBA Leave Balances sheet
      for (let i = 1; i < rbaBalancesData.length; i++) { // Skip header row
        if (rbaBalancesData[i][A_BALANCE_EMAIL] === employeeEmail) {
          if (isJanToJune) {
            ...
            if ((leaveType === "Sick Leave" && subLeaveType === "Emergency Leave") || (leaveType === "Vacation Leave" && subLeaveType === "Emergency Leave") || (leaveType === "Vacation Leave" && subLeaveType === "Unexcused") || (leaveType === "Sick Leave" && subLeaveType === "Unexcused")) {
              remainingVL = rbaBalancesData[i][A_JAN_JUN_REMAINING_VL];
              remainingSL = rbaBalancesData[i][A_JAN_JUN_REMAINING_SL];
            }
            // For "Other" leave type with Emergency/Unexcused
            if (leaveType === "Other" && (subLeaveType === "Emergency Leave" || subLeaveType === "Unexcused")) {
              remainingVL = rbaBalancesData[i][A_JAN_JUN_REMAINING_VL];
              remainingSL = rbaBalancesData[i][A_JAN_JUN_REMAINING_SL];
            }
          } else {
      ...
      for (let i = 1; i < balancesData.length; i++) { // Skip header row
    ...
    if (leaveType === "Sick Leave" || leaveType === "Vacation Leave") {
      balanceMessageEmergency = getBalanceMessage(leaveType, "Emergency Leave", (remainingSL === null && remainingVL === null) ? null : `Sick Leave: ${remainingSL}<br>Vacation Leave: ${remainingVL}`, isRBA, isJanToJune, isJulyToDec);
      balanceMessageUnexcused = getBalanceMessage(leaveType, "Unexcused", (remainingSL === null && remainingVL === null) ? null : `Vacation Leave: ${remainingVL}<br>Sick Leave: ${remainingSL}`, isRBA, isJanToJune, isJulyToDec);
    }
    let balanceMessageSick = getBalanceMessage("Sick Leave", "VL/SL", remainingSL, isRBA, isJanToJune, isJulyToDec);
    let balanceMessageVacation = getBalanceMessage("Vacation Leave", "VL/SL", remainingVL, isRBA, isJanToJune, isJulyToDec);
    ...
    } else if ((leaveType === "Vacation Leave" || leaveType === "Sick Leave") && subLeaveType === "Emergency Leave") {
      balanceMessage = balanceMessageEmergency;
    } else if ((leaveType === "Vacation Leave" || leaveType === "Sick Leave") && subLeaveType === "Unexcused") {
    ...
    if (subLeaveType === "Unexcused") {
      ...
      if (leaveType === "Other" && ((remainingVL !== null && remainingVL > 0) || (remainingSL !== null && remainingSL > 0))) {
        insufficientBalance = true;
        balanceMessage = `
        <tr class="last-row">
            <td class="info-label">Remaining Balance:</td>
            <td class="info-value">VL: ${remainingVL || 0}<br>SL: ${remainingSL || 0}</td>
        </tr>`;
      }
      
      // If "Sick Leave" is selected but VL balance is still available
      if (leaveType === "Sick Leave" && remainingVL !== null && remainingVL > 0) {
        insufficientBalance = true;
        balanceMessage = `
        <tr class="last-row">
      ...
      if (leaveType === "Vacation Leave" && (remainingVL === null || remainingVL === 0) && remainingSL !== null && remainingSL > 0) {
        insufficientBalance = true;
        balanceMessage = `
        <tr class="last-row">
            <td class="info-label">Remaining Balance:</td>
    ...
    if (subLeaveType === "Emergency Leave") {
      // If "Other" is selected but employee has available VL or SL balance
      if (leaveType === "Other" && ((remainingSL !== null && remainingSL > 0) || (remainingVL !== null && remainingVL > 0))) {
        insufficientBalance = true;
        balanceMessage = `
        <tr class="last-row">
            <td class="info-label">Remaining Balance:</td>
            <td class="info-value">SL: ${remainingSL || 0}<br>VL: ${remainingVL || 0}</td>
        </tr>`;
      }
      
      // If "Vacation Leave" is selected but SL balance is still available
      if (leaveType === "Vacation Leave" && remainingSL !== null && remainingSL > 0) {
        insufficientBalance = true;
        balanceMessage = `
        <tr class="last-row">
      ...
      if (leaveType === "Sick Leave" && (remainingSL === null || remainingSL === 0) && remainingVL !== null && remainingVL > 0) {
        insufficientBalance = true;
        balanceMessage = `
        <tr class="last-row">
            <td class="info-label">Remaining Balance:</td>
    ...
    if ((leaveType === "Sick Leave" && subLeaveType === "VL/SL") && (remainingSL === null || parsedLeaveHoursDay > remainingSL)) {
      insufficientBalance = true;
    } else if ((leaveType === "Vacation Leave" && subLeaveType === "VL/SL") && (remainingVL === null || parsedLeaveHoursDay > remainingVL)) {
      insufficientBalance = true;
    ...
    if (insufficientBalance) {
      debugLog(`Leave request automatically rejected for ${employeeFormatName} due to insufficient balance.`);
      ...
      const templateData = {
        ...
        <p>Your leave request has been <strong>automatically rejected</strong> ${
          ...
          (subLeaveType === "Unexcused" && (leaveType === "Vacation Leave" || leaveType === "Sick Leave") && 
          (remainingVL === null || remainingVL === 0) && (remainingSL === null || remainingSL === 0))
            ? "because you have <strong>no remaining VL and SL balance</strong>."
          : (subLeaveType === "Emergency Leave" && (leaveType === "Sick Leave" || leaveType === "Vacation Leave") && 
            (remainingSL === null || remainingSL === 0) && (remainingVL === null || remainingVL === 0))
            ? "because you have <strong>no remaining SL and VL balance</strong>."
          : (subLeaveType === "Unexcused" && leaveType === "Other") || (subLeaveType === "Emergency Leave" && leaveType === "Other")
          ...
          : (subLeaveType === "Unexcused" && leaveType === "Vacation Leave" && (remainingVL === null || remainingVL === 0))
            ? "because your <strong>VL balance is 0</strong>."
          : (subLeaveType === "Emergency Leave" && leaveType === "Sick Leave" && (remainingSL === null || remainingSL === 0))
                ...
                <table class="info-table" cellpadding="0" cellspacing="0">
                    ...
                    ${
                      ...
                      : (subLeaveType === "Emergency Leave" && (leaveType === "Sick Leave" || leaveType === "Vacation Leave") && 
                        (remainingSL === null || remainingSL === 0) && (remainingVL === null || remainingVL === 0))
                        ? ``
                      : (subLeaveType === "Unexcused" && (leaveType === "Sick Leave" || leaveType === "Vacation Leave"))
                        ? `
                        <tr class="card-note">
                          <td colspan="2">
                      ...
                      : (subLeaveType === "Emergency Leave" && (leaveType === "Vacation Leave" || leaveType === "Sick Leave"))
                        ? `
                        <tr class="card-note">
                          <td colspan="2">
        ...
        ${
          ...
          : (subLeaveType === "Emergency Leave" && leaveType === "Other" && (remainingSL === null || remainingSL === 0) && remainingVL > 0)
            ? `
            <div class="external-note">
              <p style="margin-bottom: 5px;"><strong>Steps to resubmit:</strong></p>
              <ul style="margin-top: 0;">
          ...
          : (subLeaveType === "Unexcused" && leaveType === "Other" && (remainingVL === null || remainingVL === 0) && remainingSL > 0)
            ? `
            <div class="external-note">
              <p style="margin-bottom: 5px;"><strong>Steps to resubmit:</strong></p>
              <ul style="margin-top: 0;">
      ...
      if ((subLeaveType === "Unexcused" || subLeaveType === "Emergency Leave") && 
      ...
      else if ((subLeaveType === "Unexcused" && leaveType === "Other") || (subLeaveType === "Emergency Leave" && leaveType === "Other")) {
        emailSubject += "Incorrect Leave Type Selection";
      } 
      // Wrong main leave type (has primary balance but used secondary)
      else if ((subLeaveType === "Unexcused" && leaveType === "Sick Leave") || (subLeaveType === "Emergency Leave" && leaveType === "Vacation Leave")) {
        emailSubject += "Wrong Main Leave Type";
      } 
      // Primary balance is 0 but secondary has balance
      else if ((subLeaveType === "Unexcused" && leaveType === "Vacation Leave" && (remainingVL === null || remainingVL === 0)) ||
              (subLeaveType === "Emergency Leave" && leaveType === "Sick Leave" && (remainingSL === null || remainingSL === 0))) {
        emailSubject += "Wrong Main Leave Type";
      }
  ...
  const templateData = {
    bodyMessage: "Kindly review the leave request and take the necessary action at your earliest convenience. Please let the employee know if you need any further details. Thank you!",
    receiverName: employeeFormatName,
...
function doGet(e) {
  ...
  const alreadyLogged = existingRecords.some((record, index) => {
    if (index + 2 === row) { // Adjust row index (Google Sheets starts at 1)
      ...
      debugLog(`Checking Row: ${row}, Status: ${existingStatus}, Forwarding: ${forwardingStatus}`);

      // ✅ Prevent duplicate forwarding if already "Forwarded"
      if (record[COL_A_EMAIL_ADDRESS] === employeeEmail && existingStartDate.toDateString() === parsedLeaveStartDate.toDateString() && existingEndDate.toDateString() === parsedLeaveEndDate.toDateString()) {
        const isProcessed = existingStatus !== "";
        const alreadyProcessed = forwardingStatus === "Forwarded" || forwardingStatus === "Not Forwarded";
        ...
        if ((action === "approve" || action === "reject") && isProcessed) {
          debugLog(`Leave already processed and cannot be forwarded: ${existingStatus}`);
  ...
  if (alreadyLogged) {
      const errorAlertTemplate = HtmlService.createTemplateFromFile('TemplateAlertUrl');
      ...
      return HtmlService.createHtmlOutput(errorAlertTemplate.evaluate().getContent());
  }
  ...
  if (action === "approve") {
    ...
    // Call updateLeaveBalances function after approval/rejection
    updateLeaveBalances();
    ...
    if (isExemptedEmail(employeeEmail)) {
      ...
      leaveBalancesSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(RBA_LEAVE_BALANCES);
      ...
      if (isJanToJune) {
        vacationUsed = employeeLeaveBalance ? employeeLeaveBalance[A_JAN_JUN_USED_VL] : 0;
        vacationRemaining = employeeLeaveBalance ? employeeLeaveBalance[A_JAN_JUN_REMAINING_VL] : 0;
        sickLeaveUsed = employeeLeaveBalance ? employeeLeaveBalance[A_JAN_JUN_USED_SL] : 0;
        sickLeaveRemaining = employeeLeaveBalance ? employeeLeaveBalance[A_JAN_JUN_REMAINING_SL] : 0;
      } else {
        vacationUsed = employeeLeaveBalance ? employeeLeaveBalance[A_JUL_DEC_USED_VL] : 0;
        vacationRemaining = employeeLeaveBalance ? employeeLeaveBalance[A_JUL_DEC_REMAINING_VL] : 0;
        sickLeaveUsed = employeeLeaveBalance ? employeeLeaveBalance[A_JUL_DEC_USED_SL] : 0;
        sickLeaveRemaining = employeeLeaveBalance ? employeeLeaveBalance[A_JUL_DEC_REMAINING_SL] : 0;
      }
      ...
      leaveBalancesSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(LEAVE_BALANCES);
      const leaveRecords = leaveBalancesSheet.getDataRange().getValues();
      ...
      vacationRemaining = employeeLeaveBalance ? employeeLeaveBalance[A_REMAINING_VL] : 0;
      sickLeaveUsed = employeeLeaveBalance ? employeeLeaveBalance[A_USED_SL] : 0;
      sickLeaveRemaining = employeeLeaveBalance ? employeeLeaveBalance[A_REMAINING_SL] : 0;
    ...
    if((leaveType === "Vacation Leave" && subLeaveType === "Unexcused") ||
    ...
    {
      logApprovedPaidLeave(row); //Log Approved Paid Leaves
    }else if((leaveType === "Other" && subLeaveType === "Unexcused") || 
    (leaveType === "Other" && subLeaveType === "Emergency Leave") ||
    subLeaveType === "Unpaid Leave" || 
    subLeaveType === "Half-day" || 
    subLeaveType === "Undertime"){
      logApprovedUnpaidLeave(row); //Log Approved Unpaid Leaves 
    }

    const approvedAlertTemplate = HtmlService.createTemplateFromFile('TemplateAlertUrl');
    ...
    return HtmlService.createHtmlOutput(approvedAlertTemplate.evaluate().getContent());
  }
  ...
  if (action === "reject") {
    ...
    logRejectedLeaveApplications(row); // Log the rejected leave to "Rejected Leave Applications" sheet
    
    const rejectAlertTemplate = HtmlService.createTemplateFromFile('TemplateAlertUrl');
    ...
    return HtmlService.createHtmlOutput(rejectAlertTemplate.evaluate().getContent());
  }
  ...
  if (action === "forward") {
    ...
    addLeaveToCalendar(employeeEmail, employeeFormatName, leaveStartDate, leaveEndDate, leaveType, supervisorName, employeeReason, noOfDaysOrHours);

    const forwardAlertTemplate = HtmlService.createTemplateFromFile('TemplateAlertUrl');
    ...
    return HtmlService.createHtmlOutput(forwardAlertTemplate.evaluate().getContent());
  }
}