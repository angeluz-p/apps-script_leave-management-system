...
function resetRequestIdCounter() {
  ...
  if (numRows > 1) { // If there are data rows (beyond header)
    ...
    PropertiesService.getScriptProperties().setProperty('lastRequestNumber_' + currentYear, maxNumber.toString());
    
    // Keep existing Request IDs - DO NOT clear Column Q
    
    Logger.log(`Request ID counter has been reset. Highest existing number: ${maxNumber}. Next submission will start from ${currentYear}-${(maxNumber + 1).toString().padStart(4, '0')}.`);
  } else {
    Logger.log("No data rows found to reset.");
  }
}
...
function generateRequestIdEnhanced(sheet) {
  ...
  if (maxNumber === 0) {
    const storedMax = PropertiesService.getScriptProperties().getProperty('lastRequestNumber_' + currentYear);
    if (storedMax) {
      maxNumber = parseInt(storedMax);
      Logger.log(`No existing request IDs found. Using stored maximum: ${maxNumber}`);
    }
  }
...
function fillMissingRequestIds() {
  ...
  if (numRows > 1) { // If there are data rows (beyond header)
    ...
    for (let i = 2; i <= numRows; i++) { // Start from row 2 (skip header)
      const existingId = sheet.getRange(i, 13).getValue();
      
      if (!existingId || existingId === '') {
        // Generate new Request ID for empty cell
        maxNumber++;
        const requestId = `${currentYear}-${maxNumber.toString().padStart(4, '0')}`;
        sheet.getRange(i, 13).setValue(requestId);
        generatedCount++;
        Logger.log(`Generated Request ID ${requestId} for row ${i}`);
      }
    }
    ...
    Logger.log(`Filled ${generatedCount} missing Request IDs. Next submission will use ${currentYear}-${(maxNumber + 1).toString().padStart(4, '0')}.`);
    console.log(`Filled ${generatedCount} missing Request IDs. Next submission will use ${currentYear}-${(maxNumber + 1).toString().padStart(4, '0')}.`);
  } else {
...
function clearStoredCounter() {
  const currentYear = new Date().getFullYear();
  PropertiesService.getScriptProperties().deleteProperty('lastRequestNumber_' + currentYear);
  Logger.log(`Cleared stored counter for year ${currentYear}. Next request will start from 0001.`);
  console.log(`Cleared stored counter for year ${currentYear}. Next request will start from 0001.`);
}